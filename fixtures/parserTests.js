...
function parserTests(parse: (file: string) => BabylonParserResult) {
  ...
  describe('File Parsing for it blocks', () => {
    it('For the simplest it cases', () => {
      ...
      const eightIt = data.itBlocks[7];
      expect(eightIt.name).toEqual('works with test.only');
      expect(eightIt.start).toEqual({column: 1, line: 30});
      expect(eightIt.end).toEqual({column: 3, line: 32});
    });
